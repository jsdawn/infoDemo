    ...
    <el-table v-loading="loadingList" v-show="showType=='list'" :data="articleList" size="small" header-cell-class-name="tableheader" stripe style="width: 100%" ref="multipleTable" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" width="100">
        <template slot-scope="scope">
          <el-button v-if="isAuth('ifo:commList:list')" type="text" size="small" @click="handleDetail(scope.$index, scope.row)">查看评论</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
    ...
    this.articleList = [
      {
        id: "222",
        title:
          "示范性高中庆祝高考圆满示范性高中庆祝高考圆满圆满示范性高中庆祝高考圆满示范性高中庆祝高考圆满",
        authorName: "千月君",
  ...
  methods: {
    ...
    handleDetail(index, row) {
      console.log(row.id);
      this.$router.push("/ifo-commContentList/"+row.id);
    },
